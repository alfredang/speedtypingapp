...
// Generates a placeholder 1024×1024 App Store icon (no alpha) on a brand gradient with a
// simple notepad + pencil motif. This is only a fallback for when you don't yet have artwork —
// always prefer your real designed icon. Customize the gradient colors to your brand palette.
//
// usage: swift make_app_icon.swift [/path/to/AppIcon-1024.png]
...
// Brand diagonal gradient — replace these two colors with your own palette.
let brandStart = NSColor(srgbRed: 0.20, green: 0.45, blue: 0.90, alpha: 1)
let brandEnd   = NSColor(srgbRed: 0.10, green: 0.20, blue: 0.45, alpha: 1)
NSGradient(starting: brandStart, ending: brandEnd)!.draw(in: full, angle: -55)
